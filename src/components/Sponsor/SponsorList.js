export const prevSponsorsList = [
    {
        title:'Github',
        logo:require('../../assets/sponsors/github.svg').default,
        url:'https://github.com/',
        cName:'sponsor-member'
    },
    {
        title:'Devfolio',
        logo: require('../../assets/sponsors/devfolio.png').default,
        url:'https://devfolio.co/',
        cName:'sponsor-member'
    },
    {
        title:'CoffeeHours',
        logo: require('../../assets/sponsors/coffeehours.png').default,
        url:'https://www.coffeehours.club/',
        cName:'sponsor-member'
    },
    {
        title:'Bugsee',
        logo: require('../../assets/sponsors/bugsee.png').default,
        url:'https://www.bugsee.com/',
        cName:'sponsor-member'
    },
    {
        title:'Clouds',
        logo:require('../../assets/sponsors/clouds.png').default,
        url:'',
        cName:'sponsor-member'
    },
    {
        title:'Matic',
        logo: require('../../assets/sponsors/matic.png').default,
        url:'https://matic.network/',
        cName:'sponsor-member'
    },
    {
        title:'Portis',
        logo: require('../../assets/sponsors/portis.png').default,
        url:'https://www.portis.io/',
        cName:'sponsor-member'
    },
    {
        title:'Fold',
        logo: require('../../assets/sponsors/fold.png').default,
        url:'https://fold.money/',
        cName:'sponsor-member'
    },
    {
        title:'Wolfram',
        logo: require('../../assets/sponsors/Wolfram.png').default,
        url:'https://www.wolfram.com/',
        cName:'sponsor-member'
    },
    {
        title:'Staise',
        logo: require('../../assets/sponsors/staise.png').default,
        url:'',
        cName:'Staise'
    },
    {
        title:'The Souled Store',
        logo: require('../../assets/sponsors/tss.png').default,
        url:'https://www.thesouledstore.com/explore/t-shirts?utm_source=google&utm_medium=cpc&utm_campaign=LD_Brand_Exact_Desktop&utm_adgroup=the_souled_store&utm_term=the%20souled%20store&utm_network=g&utm_matchtype=e&utm_device=c&gclid=CjwKCAjwzIH7BRAbEiwAoDxxThtXVbMnxXCO-R-VYyzs12mO0fJSGA3Q_v71X6XKDEFszFh0yxOMWBoCODEQAvD_BwE',
        cName:'sponsor-member'
    },
    {
        title:'Give My Certificate',
        logo:require('../../assets/sponsors/GMC.png').default,
        url:'https://givemycertificate.com/',
        cName:'sponsor-member'
    },
]

export const curSponsorsList = [
    {
        title:'Clouds',
        logo:require('../../assets/sponsors/clouds.png').default,
        url:'',
        cName:'sponsor-member'
    },
    {
        title:'Balsamiq',
        logo: require('../../assets/sponsors/balsamiq.png').default,
        url:'https://balsamiq.com/',
        cName:'sponsor-member'
    },
    {
        title:'Vue School',
        logo: require('../../assets/sponsors/vue-school.svg').default,
        url:'https://vueschool.io/',
        cName:'sponsor-member'
    },
    {
        title:'Azure',
        logo: require('../../assets/sponsors/axure.svg').default,
        url:'https://www.axure.com/',
        cName:'sponsor-member'
    },
]